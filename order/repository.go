...
func (r *postgresRepository) PutOrder(ctx context.Context, o Order) error {
	...
	stmnt, _ := tx.PrepareContext(ctx, pq.CopyIn("order_products", "order_id", "product_id", "quantity"))

	for _, p := range o.Products {
		_, err := stmnt.ExecContext(ctx, o.ID, p.ID, p.Quantity)
		if err != nil {
			return err
		}
	}

	_, err = stmnt.ExecContext(ctx)
	if err != nil {
		return err
	}
	stmnt.Close()
...
func (r *postgresRepository) GetOrdersForAccount(ctx context.Context, accountID string) ([]Order, error) {
	...
	op.product_id,
	op.quantity
	FROM orders o JOIN order_products op ON(o.id=op.order_id) where o.account_id=$1 ORDER BY o.id`, accountID,
	)
